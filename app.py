...
I18N = {
    "zh": {
        ...
        "security_info": "🔒 **零信任資安承諾**：已強制關閉 Telemetry 統計，100% 本地記憶體（RAM）處理，絕不外洩個人資料（PII），完全符合金融/企業級個資合規標準。",
        "col_in_header": "1. 輸入或上傳履歷內容",
        "file_uploader_label": "📁 上傳檔案 (.docx 或 .txt)",
        "col_in_label": "或直接貼入/編輯文字：",
...
font_choice = st.sidebar.selectbox(t["font_label"], ["Calibri", "Arial", "Times New Roman", "Microsoft JhengHei"])
...
# --- 2. 廣義標題識別庫 (支援中英文) ---
KNOWN_HEADERS = [
    "RESUME", "CURRICULUM VITAE", "PERSONAL INFORMATION", "PERSONAL DETAILS",
    "EDUCATIONAL QUALIFICATIONS", "EDUCATION", "ACADEMIC QUALIFICATIONS",
    "ACADEMIC AWARDS", "AWARDS", "HONORS AND AWARDS",
    "WORK EXPERIENCE", "EMPLOYMENT HISTORY", "CAREER HISTORY", "PROFESSIONAL EXPERIENCE",
    "OTHER SKILLS", "SKILLS", "SKILLS & CERTIFICATES", "CERTIFICATIONS",
    "JOB APPLICATION DETAILS", "APPLICATION DETAILS",
    "履歷", "個人資料", "個人信息", "聯絡資料",
    "教育背景", "教育程度", "學歷背景", "學歷資格",
    "學術獎項", "個人獎項", "獲獎紀錄",
    "工作經驗", "工作履歷", "工作經歷", "職業經歷",
    "技能與證照", "其他技能", "專業技能", "語言能力",
    "應徵資料", "求職意向", "期望薪資"
]
...
def clean_and_format_cv(raw_text):
    ...
    for line in lines:
        line_s = line.strip()
        if re.match(r'^Page\s+\d+\s+of\s+\d+$', line_s, re.IGNORECASE):
            continue
        if line_s.startswith("得出 ") or "以下已完全保留" in line_s:
            continue
...
def create_docx(raw_text, font_name, size_pt, color_rgb):
    ...
    for line in lines:
        ...
        is_header = (
            line_str.upper() in KNOWN_HEADERS or 
            (line_str.isupper() and len(line_str) < 35 and not line_str.startswith("➢"))
        )
...
def create_html(raw_text, font_name, size_pt, color_hex):
    ...
    for line in lines:
        ...
        if is_header:
            ...
            parts = line_str.split(":", 1)
            html_content += f'<p><span class="label">{parts[0]}:</span> {parts[1].strip()}</p>'
        elif "：" in line_str and len(line_str.split("：")[0]) < 25:
            parts = line_str.split("：", 1)
            html_content += f'<p><span class="label">{parts[0]}：</span> {parts[1].strip()}</p>'
        else:
...
with col_in:
    ...
    # 檔案上傳器 (支援 .docx 與 .txt)
    uploaded_file = st.file_uploader(t["file_uploader_label"], type=["txt", "docx"])
...
with col_out:
    ...
    if user_input.strip():
        ...
        # 根據編輯後的內文與自訂樣式生成 Word
        docx_data = create_docx(edited_result, font_choice, font_size, primary_rgb)
        with btn_col1:
            st.download_button(
                label=t["btn_docx"],
                data=docx_data,
        ...
        with btn_col2:
            try:
                ...
                html_str = create_html(edited_result, font_choice, font_size, primary_color_hex)
